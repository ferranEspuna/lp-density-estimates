...
\frac{u-v}{m}
\]
improves the bound to $\delta(A)\leq 4/7$ in the initial experiments (for $m=3$).
\item Further adding $(2u-v)/m$ and $(u-2v)/m$ (the \texttt{pentad\_family} in the
code) lowers the LP optimum for $m=3$ to $8/15$; see \texttt{latex/5\_experiments\_and\_bounds.tex}.
\end{itemize}
